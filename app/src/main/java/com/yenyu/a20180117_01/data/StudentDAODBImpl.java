...
public class StudentDAODBImpl implements StudentDAO {
    ...
    public ArrayList<Student> getList() {
        ...
        Cursor c = db.query("students",new String[]{"_id","name","score"},null,
                null,null,null,null);
        if(c.moveToFirst())
        {
            Student s1= new Student(c.getInt(0),
                    c.getString(1),c.getInt(2));
            mylist.add(s1);
            while(c.moveToNext())
            {
                Student s = new Student(c.getInt(0),
                        c.getString(1),c.getInt(2));
                mylist.add(s);
            }
        }
    ...
    public Student getStudent(int id) {
        ...
        if(c.moveToFirst())
        {
            Student s = new Student(c.getInt(0),c.getString(1),
                    c.getInt(2));
            return s;
        }
        return null;
